...
/**
 * Created by 오유진 on 2017-12-29.
 */

public class MannerLocXmlParser {
    public enum TagType { NONE, TITLE, CATEGORY, ADDRESS,  DESCRIPTION};

    public MannerLocXmlParser() {
    }

    ArrayList<MyLocation> parse(String xml) {

        ArrayList<MyLocation> resultList = new ArrayList();
        MyLocation dto = null;
                ...
                switch (eventType) {
                    ...
                    case XmlPullParser.START_TAG:       //시작태그를 읽음. 태그에 따라 다른 작업 수행.
                        if (parser.getName().equals("item")) {
                            dto = new MyLocation();      //새로운 dto를 생성하는 부분
